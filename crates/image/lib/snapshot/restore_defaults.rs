...
impl Manifest {
    ...
    pub fn set_restore_defaults(&mut self, defaults: RestoreDefaults) -> ImageResult<()> {
    ...
    pub fn restore_defaults(&self) -> ImageResult<RestoreDefaults> {
        ...
        let defaults: RestoreDefaults = serde_json::from_value(value.clone()).map_err(|error| {
            ImageError::ManifestParse(format!("invalid restore defaults: {error}"))
        })?;
        validate_defaults(&defaults)?;
        Ok(defaults)
    }
...
fn validate_defaults(defaults: &RestoreDefaults) -> ImageResult<()> {
    if defaults
        .user
        .as_ref()
