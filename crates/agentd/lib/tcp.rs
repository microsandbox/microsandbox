...
mod tests {
    ...
    async fn blocked_tcp_retains_data_and_eof_credit_until_consumption_or_cancel() {
        ...
        use microsandbox_protocol::core::{
            WORKLOAD_TRANSPORT_CONTROL_BYTES, WorkloadTransportCredit,
        };
        ...
        for cancel in [false, true] {
            ...
            let payload_len = WORKLOAD_TRANSPORT_CONTROL_BYTES as usize - 64;
            let data_charge = ledger.admit(InputLane::Control, payload_len + 32).unwrap();
            let eof_charge = ledger.admit(InputLane::Control, 32).unwrap();
            ...
            assert!(ledger.admit(InputLane::Control, 1).is_err());
            if cancel {
                session.close();
            ...
            assert_eq!(
                ...
                initial.control_bytes * 2
            );
            assert_eq!(ledger.credit().unwrap().control_frames, 4);
        }
    }
