...
SOCKS5 proxies now carry non-DNS UDP traffic in addition to TCP. Authenticated proxies are supported as well. SOCKS4 accepts an optional user ID, and SOCKS5 accepts a username and a password read from a host environment variable, so the password stays out of your code and shell history.
