...
For local sandboxes, status checks and runtime maintenance mark an abandoned startup as `Crashed` once neither the launcher nor the VM owns it. A normal boot stays `Starting`. Persistent disk contents are retained for recovery.
