...
SDK command calls on the same live sandbox handle can reuse a completed command's
agent connection. Concurrent commands use separate connections. At most one idle
connection is retained per handle; it closes after five idle seconds, and a
connection older than 30 seconds is not reused. Long-running commands are not
interrupted by these reuse limits.

Reused calls use the existing authenticated relay session. A new connection
performs authentication again. Stop, start, and remove calls invalidate that
handle's idle connection. Cancelled or incomplete commands discard their
connection so the relay can clean up their sessions. Commands whose result is
lost are returned as errors and are never automatically replayed.
