...
**the host runtime process environment**. The accepted range is 1–4096; when
unset, the ceiling remains 256. Invalid values prevent network initialization.
A sandbox's `maxConnections` setting can reduce this ceiling but cannot exceed
it. Guest environment variables do not configure this host policy.
...
capacity on the host before raising the ceiling. Closing sockets remain tracked
