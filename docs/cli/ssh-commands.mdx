...
SSH operations are also available as `msb sandbox ssh` and `msb sbx ssh`, including nested operations such as `msb sbx ssh serve`. The examples use the recommended short form, `msb ssh`.
...
<a id="msb-ssh-authorize"></a>

## msb sandbox ssh authorize
...
<a id="msb-ssh-serve"></a>

## msb sandbox ssh serve
...
<a id="msb-ssh-connect"></a>

## msb sandbox ssh connect
