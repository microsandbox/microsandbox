...
`layers` counts the oldest physical layers including the base, excluding the writable head. Omit it to compact all sealed layers. `last_layers: Some(n)` is an alternative to `since`; they cannot be combined with each other or `with_parents`. For direct restore, use `Sandbox::builder("child").from_snapshot("changes.msb").snapshot_base("worker:checkpoint-a")`. See [disk-chain maintenance](/sandboxes/snapshots#export-changes-and-compact-a-disk-chain) for dependency and retention rules.
...
`SandboxBuilder` setter. Create a sandbox from a snapshot artifact. Disk snapshots cold-boot; full snapshots resume captured execution. The snapshot already pins the image reference and digest, so this is mutually exclusive with [`image()`](/sdk/rust/sandbox#sandbox-image) and [`image_with()`](/sdk/rust/sandbox#sandbox-image_with). The artifact is structurally opened at [`create()`](/sdk/rust/sandbox#sandbox-create) time; persistent payload integrity is checked only through explicit [`Snapshot::verify()`](#snap-verify).
