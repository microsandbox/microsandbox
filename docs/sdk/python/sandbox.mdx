...
| external_mount_policy | `"strict" \| "relaxed"` | `"strict"` | External-filesystem admission for full restore. Inspect `await sandbox.restore_warnings()` after relaxed restore; each `ExternalMountWarning` provides `guest_path`, `reason`, and `stale_inodes`. See [Snapshot external mounts](/sandboxes/snapshots#external-mounts) |
