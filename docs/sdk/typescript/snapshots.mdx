...
Create a sandbox from a snapshot artifact. Disk snapshots cold-boot; full snapshots resume captured execution. Mutually exclusive with [`.image()`](/sdk/typescript/sandbox#sandboxbuilder-image); the snapshot already pins the image. Documented in full on the [Sandbox](/sdk/typescript/sandbox#sandboxbuilder-fromsnapshot) page.
...
const child = await Sandbox.builder("child")
  .fromSnapshot("changes.msb").snapshotBase("worker:checkpoint-a").create();
```
