...
await Snapshot.load("changes.msnap", undefined, "checkpoint-a");
const child = await Sandbox.builder("child")
  .fromSnapshot("changes.msnap").snapshotBase("checkpoint-a").create();
```
...
```typescript
await Snapshot.save("after-pip-install", "./baseline.msnap", {
  withImage: true,
});
```
