...
`CreateSandboxWithProgress` returns a bounded event channel and a single-result channel. Ignoring events does not delay creation. Always receive the result and close the sandbox when finished; cancel `ctx` to cancel creation.

```go
events, result := m.CreateSandboxWithProgress(ctx, "child", m.WithFromSnapshot("saved"), m.WithForked())
