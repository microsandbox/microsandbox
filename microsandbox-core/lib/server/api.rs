...
use crate::{
    ...
    utils::{self, MICROSANDBOX_CONFIG_FILENAME},
...
/// Server configuration for the Microsandbox API server
#[derive(Clone)]
...
impl SandboxServer {
    ...
    pub fn new(
        ...
        let server = Self {
            namespace_dir: namespace_dir.unwrap_or_else(|| {
                utils::get_microsandbox_home_path().join(utils::NAMESPACES_SUBDIR)
            }),
        ...
        if enable_default_namespace {
            ...
            let default_config_path = default_namespace_path.join(MICROSANDBOX_CONFIG_FILENAME);
    ...
    fn get_namespace_path(&self, namespace: Option<String>) -> MicrosandboxResult<PathBuf> {
    ...
    fn get_jwt_key(&self) -> MicrosandboxResult<String> {
        if let Some(key) = &self.key {
            return Ok(key.clone());
        }

        // Otherwise, we're likely a client process or the key wasn't set on startup
        Err(MicrosandboxError::SandboxServerError(
