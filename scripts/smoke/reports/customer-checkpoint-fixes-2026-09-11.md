...
The low-level follow-up is committed and pushed as libkrun `9a9aa965f65f872578f940a20d75cfea5086d834`. This Microsandbox integration uses that exact Git revision; `cargo check --locked -p microsandbox-cli --no-default-features --features net` passed without local dependency overrides. The user approved the Stop completion/timeout policy; its remaining wiring is separate from this checkpoint of the tested integration. Eager preparation deadline separation remains undecided. Formatting and diff checks passed; strict clippy remains blocked by unwired Stop scaffolding, as detailed below.
...
| 5. Stop then remove races runtime teardown | Same-run ownership completion helper and regressions exist. Request dispatch now rejects paused graceful shutdown and returns delivery errors instead of falling back to PID termination. | Public Stop/StopWithTimeout completion wiring and removal of hidden graceful-stop kill timers are approved but not yet implemented. The public waiting/timeout contract is still the old one; the pending timeout error variant is not emitted. |
| 6. Slow first forked restore times out misleadingly | Preparation progress, delayed first restore, cancellation/reaping, catalog reconciliation, and structured startup diagnostics are implemented and tested. | Eager preparation still uses the old inner Paused/activation deadlines; that timing-policy change remains pending approval. The pre-existing cancellation exposures listed below are not claimed fixed. |
...
Cleanup is not claimed universal: initial named-volume provisioning before the creation guard, pre-existing root-layer/overlay blocking workers, and cancellation of an in-flight direct-branch capture still need separate ownership audits/fixes. Bounded termination that cannot confirm exit retains durable restore intent/storage, not every in-memory lease indefinitely. Public Stop completion/timeout changes and eager preparation deadline separation have not been applied; request dispatch already rejects paused/unreachable-agent fallback termination. Adding the pending `StopTimeout` error variant is a source-compatibility change for exhaustive Rust error matches, even before any method emits it.
...
The pending, unwired local Stop completion helper produces dead-code warnings, so strict SDK clippy is not a pass. The following counts describe earlier pinned builds, not a replacement for that limitation:
...
The complete six-issue fix is not ready: public Stop ownership semantics are approved but still need integration; eager preparation/activation budget separation still requires a decision. Unplug-zero ordering and external/managed-volume providers are implemented; do not confuse their completed focused checks with universal qualification. The small-boot kernel-zone pressure and pre-existing cancellation paths above remain explicit limitations. Large-ceiling release distributions and complete Windows/Linux ARM64 live matrices are not claimed.
